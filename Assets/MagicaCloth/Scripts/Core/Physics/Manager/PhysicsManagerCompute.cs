...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerCompute : PhysicsManagerAccess
    {
        ...
        /// ボーン姿勢を元の位置に復元する
        /// </summary>
        public void UpdateRestoreBone(PhysicsTeam.TeamUpdateMode updateMode)
        {
            // 活動チームが１つ以上ある場合のみ更新
        ...
        public void UpdateSwapBuffer()
        {
            ...
            // 計算済みフラグを立てる
            foreach (var mesh in Mesh.meshSet)
            {
                if (mesh is RenderMeshDeformer)
                {
                    if (mesh.Parent.IsCalculate)
                    {
                        var state = Mesh.renderMeshStateDict[mesh.MeshIndex];
